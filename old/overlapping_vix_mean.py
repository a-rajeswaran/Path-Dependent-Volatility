...
vol_label = "VXD"
df = pd.read_excel("data/VXDDOWJ.xlsx")


#df["Return 2"] = df["Return 2"] *  df["Return 2"]
...
linear_estimate2 = utils.train_linear_model(X2, Y)
Y_pred2 = linear_estimate2.predict(X2)
Y_pred_concat = [(y1 + y2)/2 for y1, y2 in zip(Y_pred, Y_pred2)]
...
fig2 = go.Figure()
fig2.add_trace(go.Scatter(x=Dates, y=Y, mode='lines', name=f'Residual {vol_label}'))
...
Y_pred_concat_test = [(y1 + y2)/2 for y1, y2 in zip(Y_pred_test, Y_pred_test2)]
...
fig5 = go.Figure()
fig5.add_trace(go.Scatter(x=Dates_test, y=Y_test, mode='lines', name=f'Residual {vol_label}'))
