...
integral_x = torch.tensor(np.trapz(X, dx=dt, axis=1), dtype=torch.float32)
linear_estimate = utils.train_loss_function(integral_x, X, Y, dt, steps=1700)

kernel = linear_estimate[1:].detach().cpu().numpy()
...
fig.update_layout(
    title="Optimal Kernel",
    xaxis_title="Date",
    yaxis_title="Weight value",
    template="plotly_white"
)

fig.write_image("optimal_kernel.png", width=800, height=600)
fig.show()
